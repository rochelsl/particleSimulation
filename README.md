...
$U_{ij}^{\mathrm{dip}} = \frac{\mu_0}{4\pi r^3} \left[ \mathbf{m}_i \cdot \mathbf{m}_j * 3 (\mathbf{m}_i \cdot \hat{\mathbf{r}}) (\mathbf{m}_j \cdot \hat{\mathbf{r}}) \right]$
...
$$ \mathbf{F}*i = \sum*{j \ne i} \left( \mathbf{F}*{ij}^{\mathrm{LJ}} + \mathbf{F}*{ij}^{\mathrm{dip}} \right) \boldsymbol{\tau}_i = \boldsymbol{\tau}_i^{\mathrm{dip}} + \boldsymbol{\tau}_i^{\mathrm{ext}} $$
